...
| 网易云下载 · 登录后 | 网易云下载 · 下载中 |
|:---:|:---:|
| ![登录后空状态](screenshots/dl-empty.png) | ![下载进行中](screenshots/dl-busy.png) |
...
- QQ 音乐下载：扫码 / 浏览器 / Cookie 三种方式登录 → 查看并多选歌单 → 下载 MP3（保留歌名/歌手/专辑/封面）
- 命令行批量转换与桌面 GUI 两用
...
顶部切换到「网易云下载」页，可把网易云歌单歌曲下载为 MP3。

### 1. 登录

登录页提供三种方式，登录态仅保存在本次会话内；成功后界面展示账号昵称与头像，点「退出登录」即清除会话。
...
顶部切换到「QQ 音乐下载」页，登录 QQ 音乐账号后即可把歌单歌曲下载为 MP3。

### 1. 登录

登录页提供三种方式，登录态与票据仅保存在本次会话内；成功后界面展示账号昵称与头像，点「退出登录」即清除会话。

- **扫码登录**：点击「刷新二维码」后用手机 QQ「扫一扫」完成登录，并在手机上确认。登录会按 QQ 音乐网页版流程换取播放票据，账号内可播放的歌曲即可下载。
- **浏览器登录（推荐）**：自动打开本机 Chrome/Edge 进入 QQ 音乐官方登录页，在浏览器内完成登录后自动读取登录态并回到 App，无需复制粘贴；扫码遇到风控时也可在浏览器里完成登录。
- **Cookie 登录**：浏览器不可用时手动复制 Cookie 登录：先在浏览器登录 https://y.qq.com，再从开发者工具复制完整 Cookie（需包含 `uin` 与播放票据 `qm_keyst` / `qqmusic_key`）粘贴。
