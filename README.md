...
- QQ 音乐下载：QQ 扫码或 Cookie 登录 → 查看并多选歌单 → 下载 MP3（保留歌名/歌手/专辑/封面）
- 命令行批量转换与桌面 GUI 两用
...
登录页提供两种方式，登录态与票据仅保存在本次会话内；成功后界面展示账号昵称与头像，点「退出登录」即清除会话。

- **扫码登录**：点击「刷新二维码」后用手机 QQ「扫一扫」完成登录，并在手机上确认。登录会按 QQ 音乐网页版流程换取播放票据，账号内可播放的歌曲即可下载。
- **Cookie 登录**：在浏览器登录 https://y.qq.com 后，从开发者工具复制完整 Cookie（需包含 `uin` 与播放票据 `qm_keyst` / `qqmusic_key`）粘贴登录。扫码遇到风控或不便扫码时使用。
